...
Run:  .venv/Scripts/python.exe -m ml.gate_eval
...
def main():
    ...
    seg, _ = load_model(Path("runs/E5_focal/best.pth"), dev)
    ck = torch.load("runs/C1_scene/best.pth", map_location=dev, weights_only=False)
    clf = SceneNet().to(dev)
    ...
    Path("analysis/gate_eval.json").write_text(json.dumps(
        {"generated": time.strftime("%Y-%m-%dT%H:%M:%S"),
         "seg": "runs/E5_focal/best.pth", "clf": "runs/C1_scene/best.pth",
